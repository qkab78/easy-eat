...
let Cinemas = {
  getCinemas: (req,res) => {
    rp('https://maps.googleapis.com/maps/api/place/nearbysearch/json?location='+latitude+','+longitude+'&radius='+radius+'&type='+type+'&language=fr&key=AIzaSyD6Fbh72tdLpaGsMJHWTKh4Chk7PTG2v6M', (error, response, body) => {
  ...
  getCinemaInfo: (req, res) => {
    ...
    rp('https://maps.googleapis.com/maps/api/place/details/json?place_id='+req.params.id+'&language=fr&key='+apiKey, (error, response, body) => {
        console.log(body);  
        body = JSON.parse(body);
        }).then((result) => {
          result = JSON.parse(result);
